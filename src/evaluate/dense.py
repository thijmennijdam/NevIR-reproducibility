...
def calc_preferred_dense(doc1, doc2, q1, q2, model_name="dpr", model=None):
    ...
    # we added this for the dragon model
        ...
        if model is not None:
            ...
            passage_encoder = AutoModel.from_pretrained('facebook/dragon-plus-context-encoder')
    ...
    # added this code for the dragon model
    if model_type == "dragon":
        tokenizer = AutoTokenizer.from_pretrained("facebook/dragon-plus-query-encoder")
        
        # Tokenize and encode the documents
        ctx1_input = tokenizer(doc1, return_tensors="pt")
        ctx2_input = tokenizer(doc2, return_tensors="pt")
        
        ctx1_emb = passage_encoder(**ctx1_input).last_hidden_state[:, 0, :]  # CLS token embedding
        ctx2_emb = passage_encoder(**ctx2_input).last_hidden_state[:, 0, :]  # CLS token embedding

        # Compute similarity between document embeddings
        doc_sim = torch.nn.functional.cosine_similarity(ctx1_emb, ctx2_emb)

        # Process each query
        for idx, query in enumerate(queries):
            query_input = tokenizer(query, return_tensors="pt", padding=True, truncation=True, max_length=512)
            query_emb = query_encoder(**query_input).last_hidden_state[:, 0, :]  # CLS token embedding
            
            # Compute dot product scores between query and document embeddings
            scores = torch.stack([
                torch.dot(query_emb.squeeze(), ctx1_emb.squeeze()),
                torch.dot(query_emb.squeeze(), ctx2_emb.squeeze())
            ])
            
            results[f"q{idx+1}"] = scores.tolist()
            
            # Check if the score matches expectations
            should_be_higher = scores[idx]
            should_be_lower = scores[0] if idx != 0 else scores[1]
            if should_be_higher > should_be_lower:
                num_correct += 1
        model = (query_encoder, passage_encoder)
        ...
        for idx, query in enumerate(queries):
            ...
            # COEN ADJUSTED THIS TO BE FIXED WITH NLI TASKS
            # [[-1.7144152 -3.4611146  5.4999824][-1.4242139 -3.5668974  5.377844 ]] for nli-deberta
            # check if scores is a list of lists
            if scores[0] is not None and type(scores[0]) == list: # THIS LINE
