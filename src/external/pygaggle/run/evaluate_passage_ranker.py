...
from external.pygaggle.run.args import ArgumentParserBuilder, opt
from external.pygaggle.rerank.base import Reranker
from external.pygaggle.rerank.bm25 import Bm25Reranker
from external.pygaggle.rerank.transformer import (
    UnsupervisedTransformerReranker,
    MonoT5,
    DuoT5,
    MonoBERT
)
from external.pygaggle.rerank.random import RandomReranker
from external.pygaggle.rerank.similarity import CosineSimilarityMatrixProvider
from external.pygaggle.model import (SimpleBatchTokenizer,
                            RerankerEvaluator,
                            DuoRerankerEvaluator,
                            metric_names,
                            MsMarcoWriter)
from external.pygaggle.data import MsMarcoDataset
from external.pygaggle.settings import MsMarcoSettings
...
def construct_t5(options: PassageRankingEvaluationOptions) -> Reranker:
    ...
    tokenizer_name = options.tokenizer_name if options.tokenizer_name else options.model
    hf_tokenizer = AutoTokenizer.from_pretrained(
        tokenizer_name,
        use_fast=False
    )

    # Wrap the Hugging Face tokenizer with SimpleBatchTokenizer
    # so it supports `traverse_query_document`
...
def main():
    ...
    construct_map = dict(transformer=construct_transformer,
                         ...
                         seq_class_transformer=construct_t5,
