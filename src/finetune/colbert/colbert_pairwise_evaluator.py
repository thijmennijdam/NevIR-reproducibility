...
class PairwiseEvaluator(TripletEvaluator):
    ...
    def __call__(self, model, output_path: str = None, epoch: int = -1, steps: int = -1) -> float:
        ...
        for idx in range(num_instances):
            ...
            results, model, _ = calc_preferred_colbert(self.doc1s[idx], self.doc2s[idx], self.q1s[idx], self.q2s[idx], model=model)

           
            if idx%100==0:
                
                print(f'result at idx {idx} = {results["score"]}')
        ...
        print(f'paired_accuracy at epoch {epoch} = {paired_accuracy}')
