...
from evaluate.rerank_pygaggle import calc_preferred_rerankers
...
class PairwiseEvaluator(TripletEvaluator):
    ...
    def __call__(self, model, output_path: str = None, epoch: int = -1, steps: int = -1) -> float:
        ...
        for idx in range(num_instances):
            results, model, _ = calc_preferred_rerankers(self.doc1s[idx], self.doc2s[idx], self.q1s[idx], self.q2s[idx], model_name=self.model_name, model=model)
