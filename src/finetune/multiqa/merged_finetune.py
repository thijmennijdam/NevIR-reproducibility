...
from finetune.pairwise_evaluator import PairwiseEvaluator
...
def load_msmarco_topk1000_dev(data_folder: str):
    ...
    Returns:
        Tuple[Dict[str, str], Dict[str, str], Dict[str, Set[str]]]: Tuple containing the dev queries, the corpus, and the relevant documents per query.
    """
...
def main():
    ...
    parser.add_argument("--merged_path", type=str, default="data/merged_dataset/", help="Path to merged dataset.")
    parser.add_argument("--output_path", type=str, default="multiqa/finetune_merged/", help="Path to save the model.")
    parser.add_argument("--epochs", type=int, default=20, help="Number of epochs to train.")
    parser.add_argument("--lr", type=float, default=2e-5, help="Learning rate for training.")
    parser.add_argument("--train_triplets", type=str, default="combined_train_triplets.tsv", help="File containing the training triplets.")
    ...
    print("Loading NevIR validation data...")
    ...
    all_samples = []
    with open(args.merged_path + args.train_triplets, "r") as f:
        for line in f:
            query, positive, negative = line.strip().split("\t")
            all_samples.append(InputExample(texts=[query, positive, negative]))

    train_dataset = SentencesDataset(all_samples, model)
